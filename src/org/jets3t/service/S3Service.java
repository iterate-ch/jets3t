...
public abstract class S3Service extends RestStorageService implements SignedUrlHandler {
     ...
     *  <b>Note 2:</b> For signature version "AWS4-HMAC-SHA256" this time cannot be more
     *  than 7 days into the future.
     * @param isVirtualHost
     * if this parameter is true, the bucket name is treated as a virtual host name. To use
     * this option, the bucket name must be a valid DNS name that is an alias to an S3 bucket.
     * @param isHttps
     ...
     */
    public String createSignedUrlUsingSignatureVersion(
        String requestSignatureVersion, String region,
        ...
        boolean isVirtualHost, boolean isHttps, boolean isDnsBucketNamingDisabled)
    {
        ...
        if (specialParamName != null && specialParamName.length() > 0) {
        ...
        if ("AWS4-HMAC-SHA256".equalsIgnoreCase(requestSignatureVersion)) {
            ...
            Map<String, Object> renamedHeadersMap = renameMetadataKeys(headersMap);
            Map<String, String> headersToSignMap = new TreeMap<String, String>();
            for (String headerName: renamedHeadersMap.keySet()) {
                ...
                String value = renamedHeadersMap.get(headerName) == null
                    ? "" : renamedHeadersMap.get(headerName).toString().trim();
                headersToSignMap.put(name, value);
            }
            ...
            throw new RuntimeException(
                "Unsupported value for request signature version \""
    ...
    public String createSignedUrl(String method, String bucketName, String objectKey,
        ...
        boolean isVirtualHost, boolean isHttps, boolean isDnsBucketNamingDisabled)
    {
        return createSignedUrlUsingSignatureVersion(
            "AWS2", null, method, bucketName, objectKey,
     ...
     * @throws S3ServiceException
     */
    public MultipleDeleteResult deleteMultipleObjects(String bucketName, String[] keys)
        throws S3ServiceException
    {
     ...
     * @throws S3ServiceException
     */
    public MultipleDeleteResult deleteMultipleObjects(String bucketName, ObjectKeyAndVersion[] objectNameAndVersions)
        throws S3ServiceException
    {
     ...
     * @throws S3ServiceException
     */
    public MultipleDeleteResult deleteMultipleObjects(
        String bucketName, ObjectKeyAndVersion[] objectNameAndVersions,
        boolean isQuiet) throws S3ServiceException
    {
        return deleteMultipleObjectsWithMFA(bucketName, objectNameAndVersions, null, null, isQuiet);
    }
     ...
     * @throws S3ServiceException
     */
    public MultipleDeleteResult deleteMultipleObjectsWithMFA(
        String bucketName, ObjectKeyAndVersion[] objectNameAndVersions,
        String multiFactorSerialNumber, String multiFactorAuthCode,
        boolean isQuiet) throws S3ServiceException
    {
        return deleteMultipleObjectsWithMFAImpl(bucketName, objectNameAndVersions,
            multiFactorSerialNumber, multiFactorAuthCode, isQuiet);
    }
